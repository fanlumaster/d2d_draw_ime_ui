...
// Link the static library (make sure that file is in the same directory as this
// file)
...
// The function you call to set up the above options.  Make sure its called
// before the DirectOverlaySetup function
...
// Initializes a the overlay window, and the thread to run it.  Input your
// callback function. Uses the first window in the current process to target. If
// you're external, use the next function
...
// Draws a rectangle on the screen.  Width and height are relative to the
// coordinates of the box. Use the "filled" bool to make it a solid rectangle;
// ignore the thickness. To just draw the border around the rectangle, specify a
// thickness and pass "filled" as false.
void DrawBox(float x, float y, float width, float height, float thickness,
             float r, float g, float b, float a, bool filled);

// Draws a circle.  As with the DrawBox, the "filled" bool will make it a solid
// circle, and thickness is only used when filled=false.
void DrawCircle(float x, float y, float radius, float thickness, float r,
                float g, float b, float a, bool filled);

// Allows you to draw an elipse.  Same as a circle, except you have two
// different radii, for width and height.
